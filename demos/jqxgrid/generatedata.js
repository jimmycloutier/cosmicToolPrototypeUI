function generateFP(rowscount) {
  ...
  var refFP =
  [
    "SRS p. 21", "SRS p. 25", "SRS p. 70"
  ];

  var functionProcesses =
  [
    "Create user", "Retrieve user", "Update user", "Delete user", "List user"
  ];

  var remarksFP =
  [
    "Lorem ipsum dolor sit", "consectetur adipiscing elit", "Aliquam lacus lacus", "Praesent molestie leo eu ante sagittis auctor"
  ];
  ...
  for (var i = 0; i < rowscount; i++) {
        ...
        row["functionalProcess"] = functionProcesses[Math.floor(Math.random() * functionProcesses.length)];
        row["remark"] = remarksFP[Math.floor(Math.random() * remarksFP.length)];
...
function generateDG(rowscount) {
  ...
  var dataGroups =
  [
    "DG1", "DG2", "DG3", "DG4", "DG5", "DG6", "Message", "Filter", "Error message"
  ];

  var movements =
  [
    "E", "W", "R", "X", "RX", "EW", "EX", "ER", "WX"
  ];
